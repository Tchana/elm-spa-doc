module Ui exposing (hero, colors, markdown, button, transition, sidebar)
...
button ( label, url ) =
    link [ Background.color colors.white
...
sidebar : { header : String, sections : List Section, active : Route, links : List (String, Route) } -> Element msg
sidebar options =
    column
        [ spacing 16 ]
...
viewLink activeRoute ( label, route ) =
    link [ Font.underline
        ...
        { url = Routes.toPath route, label = text label }